...
// import comercio from "./datos/privados/comercio_130_75.json"; // ya no usamos JSON local
import { loadEscalasFromSheets } from "./utils/loadEscalasFromSheets.js";

export default function CalculadoraSueldoTandil() {
  // Estado UI
  const [sector, setSector] = useState("publico");       // publico | privado
  const [convenio, setConvenio] = useState("municipalidad"); // municipalidad | comercio (privado)
  const [subRegimen, setSubRegimen] = useState("administracion"); // administración | sisp | obras
  ...
  // Escalas dinámicas (Comercio desde Google Sheets)
  const [comercioEscalas, setComercioEscalas] = useState(null);

  useEffect(() => {
    loadEscalasFromSheets().then(setComercioEscalas).catch(() => setComercioEscalas({}));
  }, []);

  // Agrupo sector público bajo un solo convenio "Municipalidad de Tandil"
  const publicoFuentes = useMemo(() => ({
    administracion: municipio,   // Administración Central
    sisp: sisp,                  // SISP
    obras: obras,                // Obras Sanitarias
  }), []);

  // Qué datos usar según selección
  const datosMunicipalidadActual = useMemo(() => {
    return publicoFuentes[subRegimen] || municipio;
  }, [publicoFuentes, subRegimen]);

  // Mapa de convenios por sector
  const convenios = useMemo(() => {
    const publico = {
      municipalidad: { nombre: "Municipalidad de Tandil", escalas: datosMunicipalidadActual.escalas }
    };

    const privado = comercioEscalas
      ? { comercio: { nombre: "Empleados de Comercio", escalas: comercioEscalas } }
      : {}; // mientras carga, vacío

    return { publico, privado };
  }, [datosMunicipalidadActual, comercioEscalas]);

  // Accesos
  const datosConvenio = (convenios[sector] && convenios[sector][convenio]) || null;
  ...
  // Al cambiar convenio / subRegimen → fijar mes/categoría/régimen válidos en forma segura
  useEffect(() => {
    const fuenteEscalas = datosConvenio?.escalas;
    if (!fuenteEscalas || typeof fuenteEscalas !== "object") {
      setMes("");
      return;
    }
    const meses = Object.keys(fuenteEscalas);
    if (meses.length === 0) {
      setMes("");
      return;
    }
  ...
  const ready = !tieneEscalas || Boolean(escalaActual);
  ...
  if (sector === "publico") {
    ...
    if (escalaActual) {
      ...
      // NR fijo por mes si existiera en la escala (viene de la hoja, opcional)
    ...
    // Bases de cálculo correctas (según tu indicación)
  ...
  const mesesDisponibles = tieneEscalas ? Object.keys(datosConvenio?.escalas || {}).sort() : [];
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
        ...
        <section className="bg-white p-5 rounded-xl shadow">
          ...
          {/* Sub-régimen solo para Municipalidad */}
        ...
        <section className="bg-white p-5 rounded-xl shadow">
          ...
          {!ready ? (
            sector === "privado" && !comercioEscalas ? (
              <p className="p-3 rounded bg-yellow-50 border border-yellow-200 text-yellow-800">
